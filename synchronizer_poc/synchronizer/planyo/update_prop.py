...
class UpdateUserComment:
    ...
    def update_prop_user_comment(self, user_id, comment):
        old_comment = self.api_call({'method': 'get_user_data', 'user_id': user_id, 'detail_level': 4}).json()['data'][
            'properties']['comment']
        print('changing user_prop_comment from "{}" to "{}" for user'.format(old_comment, comment, user_id))
